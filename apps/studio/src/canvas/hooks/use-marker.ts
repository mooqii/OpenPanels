...
interface UseMarkerOptions {
  editor: Editor
  previewShapeRef: RefObject<Konva.Shape | null>
}

export interface MarkerPreviewShape {
  color: string
  opacity: number
  points: number[]
  size: number
...
export function useMarker({ editor, previewShapeRef }: UseMarkerOptions) {
  const isDrawingRef = useRef(false)
  const pointsRef = useRef<number[]>([])
  const [previewShape, setPreviewShape] = useState<MarkerPreviewShape | null>(
    null
  )
  ...
  const handleMouseDown = useCallback(
    (_e: KonvaEventObject<PointerEvent>) => {
      ...
      // Start with the point doubled (for single-click visibility per Konva docs)
      const initialPoints = [pos.x, pos.y, pos.x, pos.y]

      // Create preview shape (not yet in store)
      setPreviewShape({
