...
import type { OpenPanelsTransport } from "../types"
...
export function transportKey(transport: OpenPanelsTransport | null): string {
  if (!transport) return "none"
  return `http:${transport.apiBase}`
}

export function useOpenPanelsTransport(): OpenPanelsTransport | null {
