...
interface MarkerCursorPreviewState {
  color: string
  diameter: number
  x: number
  y: number
...
function createPencilCursor(color: string): string {
  const svg = `
...
export function Canvas({
  ...
  const transform = useTransform(editor)
  ...
  return (
    <div
      ...
      style={{
        ...
        height,
        position: "relative",
        outline: dragdrop.isDragging ? "2px dashed #3b82f6" : undefined,
        outlineOffset: "-2px",
    ...
    >
      ...
      {/* Native textarea for text editing (outside Konva Layer) */}
      <textarea
...
function summarizeSelectionShape(
