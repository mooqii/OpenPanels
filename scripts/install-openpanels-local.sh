...
REPO="${OPENPANELS_INSTALL_REPO:-mooqii/OpenPanels}"
DEFAULT_MANIFEST_URL="https://github.com/$REPO/releases/latest/download/openpanels-local-manifest.json"
MANIFEST_URL="${OPENPANELS_INSTALL_MANIFEST_URL:-${OPENPANELS_UPDATE_MANIFEST_URL:-$DEFAULT_MANIFEST_URL}}"
INSTALL_DIR="${OPENPANELS_INSTALL_DIR:-$HOME/.local/bin}"
BINARY_NAME="openpanels-local"

fail() {
  printf 'openpanels-local install failed: %s\n' "$*" >&2
  exit 1
}
...
ARCHIVE_PATH="$TMP_DIR/openpanels-local-asset"
EXTRACT_DIR="$TMP_DIR/extract"

printf 'Installing openpanels-local for %s\n' "$TARGET"
