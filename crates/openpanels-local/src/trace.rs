...
pub fn event_for_audience(event: &Value, audience: &str) -> Option<Value> {
    ...
    Some(json!({
        ...
        "category": category,
        "source": event.get("source").cloned().unwrap_or_else(|| json!("openpanels")),
...
pub fn emit_cli_event(input: TraceEventInput) {
    let Ok(url) = std::env::var("OPENPANELS_TRACE_URL") else {
        return;
    };
...
fn normalize_external_event(input: TraceEventInput) -> Value {
    ...
    let summary = sanitize_text(
        input
            .summary
            .unwrap_or_else(|| "OpenPanels activity".to_owned()),
    );
    ...
    json!({
        "timestamp": now_iso(),
        "category": category,
        "source": sanitize_short(input.source.as_deref().unwrap_or("openpanels")),
        ...
        "runId": input.run_id.or_else(|| std::env::var("OPENPANELS_TRACE_RUN_ID").ok()).map(|value| sanitize_short(&value)),
