...
pub(super) fn agent_targets_path(paths: &OpenPanelsPaths) -> PathBuf {
    paths.context_dir.join("agent-targets.json")
}

pub(super) fn read_agent_targets(paths: &OpenPanelsPaths) -> Result<Vec<Value>, CliError> {
...
pub(super) fn trace_task_event(
    ...
    trace::record(TraceEventInput {
        ...
        release_summary,
        run_id: env::var("OPENPANELS_TRACE_RUN_ID").ok(),
