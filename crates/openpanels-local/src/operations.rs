use crate::agent::read_agent_guide;
use crate::canvas::{
...
pub fn list(paths: &OpenPanelsPaths, status: Option<&str>) -> Result<Value, CliError> {
    let operations =
        Storage::open(paths)?.list_agent_operations(Some(&paths.context_id), status)?;
    Ok(json!({ "operations": operations }))
}

pub fn inspect(paths: &OpenPanelsPaths, id: &str) -> Result<Value, CliError> {
    Storage::open(paths)?
...
fn active_operation(paths: &OpenPanelsPaths, id: &str, intent: &str) -> Result<Value, CliError> {
...
fn save(paths: &OpenPanelsPaths, operation: &Value) -> Result<(), CliError> {
...
pub fn begin_canvas(
    ...
    let now = now_iso();
    let guide = read_agent_guide(paths, "canvas.image-generation", None)?;
    let operation = json!({
        "id": id,
        ...
        "panelKind": "canvas",
        "guideId": "canvas.image-generation",
    ...
    Ok(
        json!({ "operation": operation, "guide": guide, "nextAction": "Generate the bitmap, then run canvas generation complete." }),
    )
}
...
pub fn begin_wiki(
    ...
    let now = now_iso();
    let guide = read_agent_guide(paths, "wiki.generated-documents", None)?;
    let operation = json!({
        ...
        "guideId": "wiki.generated-documents", "protocolVersion": AGENT_PROTOCOL_VERSION,
    ...
    save(paths, &operation)?;
    Ok(
        json!({ "operation": operation, "guide": guide, "document": started["document"], "nextAction": "Write the result file, then run wiki generation complete." }),
    )
}

pub fn complete_wiki(paths: &OpenPanelsPaths, id: &str, file: &str) -> Result<Value, CliError> {
...
mod tests {
    ...
    fn test_paths() -> (tempfile::TempDir, OpenPanelsPaths) {
