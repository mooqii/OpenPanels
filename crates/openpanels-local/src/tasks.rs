...
pub fn list_targets(paths: &OpenPanelsPaths) -> Result<Value, CliError> {
    let bootstrap = read_project_bootstrap(paths, BootstrapRequest::new())?;
    import_legacy_targets(paths, &bootstrap.session.id)?;
...
pub fn heartbeat_target(paths: &OpenPanelsPaths, target_id: &str) -> Result<Value, CliError> {
    let bootstrap = read_project_bootstrap(paths, BootstrapRequest::new())?;
...
pub fn remove_target(paths: &OpenPanelsPaths, target_id: &str) -> Result<Value, CliError> {
    let bootstrap = read_project_bootstrap(paths, BootstrapRequest::new())?;
    let storage = Storage::open(paths)?;
    let changed = storage
        .connection()
...
pub fn list_deliveries(paths: &OpenPanelsPaths, task_id: Option<&str>) -> Result<Value, CliError> {
    let bootstrap = read_project_bootstrap(paths, BootstrapRequest::new())?;
    let storage = Storage::open(paths)?;
...
fn recover_expired_tasks(paths: &OpenPanelsPaths) -> Result<(), CliError> {
...
fn read_targets(paths: &OpenPanelsPaths, session_id: &str) -> Result<Vec<Value>, CliError> {
    let storage = Storage::open(paths)?;
...
pub fn start_dispatcher_loop(paths: OpenPanelsPaths, server_url: String) {
...
pub fn dispatcher_status(paths: &OpenPanelsPaths) -> Result<Value, CliError> {
...
fn deliver_webhook(
    ...
    let response = ureq::post(endpoint)
        ...
        .set("x-openpanels-target-id", target_id)
        .set("x-openpanels-signature", &format!("sha256={signature}"))
...
fn import_legacy_targets(paths: &OpenPanelsPaths, session_id: &str) -> Result<(), CliError> {
...
fn target_secret_path(paths: &OpenPanelsPaths, target_id: &str) -> std::path::PathBuf {
...
fn read_target_secret(paths: &OpenPanelsPaths, target_id: &str) -> Result<String, CliError> {
