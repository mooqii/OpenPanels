fn read_typesetting_task(paths: &MyOpenPanelsPaths, task_id: &str) -> Result<Value, CliError> {
    ...
    if task.get("queue").and_then(Value::as_str) != Some("typesetting")
        || !task
            ...
            .is_some_and(is_typesetting_task_type)
    {
...
pub fn claim_task(paths: &MyOpenPanelsPaths, task_id: &str) -> Result<Value, CliError> {
    let mut payload = read_typesetting_task(paths, task_id)?;
...
pub fn heartbeat_task(paths: &MyOpenPanelsPaths, task_id: &str) -> Result<Value, CliError> {
    read_typesetting_task(paths, task_id)
}
...
pub(crate) fn prepare_task_completion(
    ...
    let payload = read_typesetting_task(paths, task_id)?;
    let task = &payload["task"];
