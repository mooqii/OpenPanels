...
mod tests {
    ...
    fn cover_request_snapshots_article_and_skill_and_is_idempotent() {
        ...
        assert_eq!(
            ...
            "handler.typesetting.cover-generation"
        );
        assert!(instructions.contains("references/typesetting-contract.md"));
        assert!(instructions.contains("references/typesetting-cover-generate.md"));
        assert!(instructions.contains(&format!("Task id: `{task_id}`")));
        assert!(instructions.contains(&format!("Task type: `{COVER_TASK_TYPE}`")));
        assert!(instructions.contains(&format!("Task capability: `{COVER_CAPABILITY}`")));
        ...
        assert!(message.contains("references/typesetting-cover-generate.md"));
    ...
    #[test]
    fn title_request_generates_ten_candidates_without_changing_selection() {
        let (_temp, paths) = test_paths();
        ...
        assert_eq!(task["capability"], json!(TITLE_CAPABILITY));
        assert_eq!(task["maxAttempts"], json!(3));
        ...
        assert_eq!(
            prepared.bundle.handler_key,
            "handler.typesetting.title-generation"
        );
        assert!(prepared
            .bundle
            .instructions
            .contains("references/typesetting-title-generate.md"));
        assert!(prepared
            .bundle
            .instructions
        ...
        assert!(prepared
            .bundle
            .instructions
            .contains("exactly 10 distinct"));
        assert_eq!(
            prepared.bundle.output_contract["artifacts"][0]["role"],
            json!("typesetting-titles")
        );
    ...
    fn layout_request_snapshots_content_and_excludes_concurrent_work() {
        ...
        assert_eq!(task["capability"], json!(LAYOUT_CAPABILITY));
        assert_eq!(task["maxAttempts"], json!(1));
        ...
        assert_eq!(
            prepared.bundle.handler_key,
            "handler.typesetting.content-layout"
        );
        assert!(instructions.contains("references/typesetting-contract.md"));
        assert!(instructions.contains("references/typesetting-content-format.md"));
        assert!(instructions.contains(&format!("Task id: `{task_id}`")));
        assert!(instructions.contains(&format!("Task type: `{LAYOUT_TASK_TYPE}`")));
        assert!(instructions.contains(&format!("Task capability: `{LAYOUT_CAPABILITY}`")));
        assert!(instructions.contains(&format!("Project id: `{}`", bootstrap.project.id)));
        assert!(instructions.contains(&format!("Panel id: `{typesetting_panel}`")));
        ...
        assert!(message.contains("task handoff heartbeat --handoff-id"));
        assert!(message.contains("task handoff complete --handoff-id"));
        assert!(message.contains("references/typesetting-content-format.md"));
        ...
        assert_eq!(conflict.code(), Some("typesetting_layout_in_progress"));
        ...
        assert_eq!(locked.code(), Some("typesetting_content_locked"));
    }
