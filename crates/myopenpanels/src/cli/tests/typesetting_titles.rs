#[test]
fn typesetting_title_commands_list_skills_and_create_a_task() {
    ...
    let mut list_args = vec!["typesetting", "title", "skill", "list"];
    ...
    assert!(skills.as_array().is_some_and(|items| items.iter().any(|item| {
        item.pointer("/skill/id").and_then(Value::as_str)
            == Some(crate::typesetting::DEFAULT_TITLE_SKILL_ID)
    })));
    ...
    assert_eq!(
        payload["task"]["type"],
        json!(crate::typesetting::TITLE_TASK_TYPE)
    );
    assert_eq!(
        payload["task"]["input"]["titleSkillId"],
        json!(crate::typesetting::DEFAULT_TITLE_SKILL_ID)
    );
    assert_eq!(
