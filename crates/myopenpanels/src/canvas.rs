...
pub(crate) fn prepare_placeholder_for_target(
    ...
    store.insert(
        ...
        json!({
            ...
            "meta": {
                "myopenpanelsGenerationPlaceholder": true,
                "createdAt": now_iso(),
            },
        }),
    );
...
fn canvas_bootstrap(paths: &MyOpenPanelsPaths) -> Result<crate::types::ProjectBootstrap, CliError> {
...
fn load_canvas_target(
    ...
    let (state, revision) = storage
        ...
        .ok_or_else(|| {
            CliError::with_code(
                "target_not_found",
                format!("Canvas state not found: {panel_id}"),
            )
        })?;
    Ok(CanvasTarget {
        project,
...
pub(crate) fn prepare_placeholder_update_for_target(
    ...
    if remove {
        ...
        shape["props"]["text"] = json!(text.unwrap_or("生成失败"));
