...
pub fn list_agent_targets(paths: &MyOpenPanelsPaths) -> Result<Value, CliError> {
...
pub(super) fn wake_queued_wiki_tasks(
    ...
    if env::var("MYOPENPANELS_ENABLE_LEGACY_LOCAL_AGENT")
        .ok()
        .as_deref()
        != Some("1")
    {
...
fn wiki_wakeup_message(
    ...
    json!({
        ...
        "wikiAgentSkillId": task
            .get("agentSkillId")
            .and_then(Value::as_str)
            .unwrap_or_else(|| super::selected_agent_skill_id(&wiki.state)),
        "createdAt": now_iso(),
...
fn wake_local_agent_worker(
    ...
    command
        .args(&worker.args)
        .env("MYOPENPANELS_AGENT_WORKER", "1")
        .env("MYOPENPANELS_PROJECT_DIR", &paths.project_dir)
        .env("MYOPENPANELS_STORAGE_DIR", &paths.storage_dir)
        .env("MYOPENPANELS_TRACE_RUN_ID", task_id)
        .env("MYOPENPANELS_TRACE_AUDIENCE", "development")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
...
fn resolve_local_agent_worker(
    ...
    let requested = env::var("MYOPENPANELS_LOCAL_AGENT_HOST").ok();
    let codex = find_executable("codex", env::var("MYOPENPANELS_CODEX_EXECUTABLE").ok());
    let hermes = find_executable("hermes", env::var("MYOPENPANELS_HERMES_EXECUTABLE").ok());
...
fn should_wake_local_agent_worker(allow_local_worker_from_agent_worker: bool) -> bool {
    ...
    if env::var("MYOPENPANELS_DISABLE_LOCAL_AGENT").ok().as_deref() == Some("1") {
        return false;
    }
    if cfg!(test) || env::var("NODE_ENV").ok().as_deref() == Some("test") {
        return false;
    }
    if env::var("MYOPENPANELS_AGENT_WORKER").ok().as_deref() == Some("1")
        && !allow_local_worker_from_agent_worker
    {
        return false;
    }
    env::var("MYOPENPANELS_ENABLE_LOCAL_AGENT").ok().as_deref() == Some("1")
...
2. Load the Wiki panel operating contract:
   {} {} agent skill wiki-panel --task-id {}
   Then read the returned localPath (`SKILL.md`) and
   `references/authoring-skill-routing.md`.
3. Resolve the selected authoring skill package and task context:
   {} {} agent skill {} --task-id {}
...
fn preferred_local_cli_command() -> String {
    if let Ok(cli) = env::var("MYOPENPANELS_CLI") {
