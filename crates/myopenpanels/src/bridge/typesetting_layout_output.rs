fn build_typesetting_layout_output_plan(
    ...
    let result = read_execution_result_v2(workspace, "Typesetting Layout")?;
    validate_result_keys(
        &result,
        &["schemaVersion", "outcome", "summary", "artifacts"],
        "Typesetting Layout",
    )?;
    require_outcome(&result, "formatted", "Typesetting Layout")?;
    let artifact = exactly_one_artifact(workspace, &result, "Typesetting Layout")?;
    ...
    .map_err(|_| {
        CliError::with_code(
            "invalid_output",
            "Typesetting Layout output must be valid JSON.",
        )
    })?;
    ...
    if layout_text(source) != layout_text(&content) {
        return Err(CliError::with_code(
            "invalid_output",
            "Typesetting Layout cannot change publication text.",
        ));
    }
    if layout_images(source) != layout_images(&content) {
        return Err(CliError::with_code(
            "invalid_output",
            "Typesetting Layout cannot add, remove, reorder, or change images.",
        ));
    }
    if layout_links(source) != layout_links(&content) {
        return Err(CliError::with_code(
            "invalid_output",
            "Typesetting Layout cannot add, remove, or change links.",
        ));
    }
    let task_id = task
        .get("id")
...
fn validate_layout_document(document: &Value) -> Result<(), CliError> {
    if document.get("type").and_then(Value::as_str) != Some("doc") {
        return Err(CliError::with_code(
            "invalid_output",
            "Typesetting Layout output must be a TipTap doc.",
        ));
    }
...
fn validate_layout_node(node: &Value) -> Result<(), CliError> {
    ...
    if !matches!(
        ...
        return Err(CliError::with_code(
            "invalid_output",
            format!("Typesetting Layout output contains unsupported node: {node_type}"),
        ));
    }
        ...
        && !matches!(node.pointer("/attrs/level").and_then(Value::as_i64), Some(1..=3))
    {
        return Err(CliError::with_code(
            "invalid_output",
            "Typesetting Layout headings must use levels 1 through 3.",
        ));
    }
    if node_type == "text" && !node.get("text").is_some_and(Value::is_string) {
        return Err(CliError::with_code(
            "invalid_output",
            "Typesetting Layout text nodes require text.",
        ));
    }
    if node_type == "image" && !node.get("attrs").is_some_and(Value::is_object) {
        return Err(CliError::with_code(
            "invalid_output",
            "Typesetting Layout image nodes require attributes.",
        ));
    }
    for mark in node
        .get("marks")
        ...
        .flatten()
    {
        ...
        if !matches!(mark_type, "bold" | "italic" | "link") {
            return Err(CliError::with_code(
                "invalid_output",
                format!("Typesetting Layout output contains unsupported mark: {mark_type}"),
            ));
        }
        if mark_type == "link"
            && !mark
                ...
                .is_some_and(|href| !href.is_empty())
        {
            return Err(CliError::with_code(
                "invalid_output",
                "Typesetting Layout links require href attributes.",
            ));
        }
    }
    for child in node
...
#[cfg(test)]
mod typesetting_layout_output_tests {
    use super::*;
    ...
    fn layout_output_preserves_text_images_and_links() {
        ...
        assert!(build_typesetting_layout_output_plan(
            &paths,
            &task,
            &workspace,
            "attempt:1",
    ...
    fn layout_output_rejects_unsupported_nodes_changed_images_and_links() {
        ...
        for invalid in [unsupported, changed_image, changed_link] {
            ...
            .expect("content");
            write_layout_result(&workspace, "outputs/content.json");
            let error = build_typesetting_layout_output_plan(
                &paths,
                &task,
                &workspace,
                "attempt:1",
