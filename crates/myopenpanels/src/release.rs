...
pub fn publishing_task_capability(task_type: &str) -> Result<&'static str, CliError> {
    Ok(
        &crate::capabilities::task_route_for_capability(PUBLISHING_TASK_CAPABILITY_KEY, task_type)?
            .capability,
    )
}
...
pub fn create_release(
    ...
    let body_text = publication_plain_text(publication.get("content").unwrap_or(&Value::Null));
...
fn build_attempt(
    ...
    let mut task = TaskInsert::for_capability(
        PUBLISHING_TASK_CAPABILITY_KEY,
        target.task_type,
        task_id,
        release_id.to_owned(),
        json!({
    ...
    task.idempotency_key = Some(request_id.to_owned());
    Ok((attempt, task))
}
...
pub fn publication_plain_text(document: &Value) -> String {
...
mod tests {
    ...
    fn plain_text_preserves_lists_and_excludes_images() {
        ...
        let text = publication_plain_text(&document);
