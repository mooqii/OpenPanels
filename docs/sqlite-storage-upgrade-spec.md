...
CreartCLI 的数据库层提供了一个更适合本地项目的参考模型：用 SQLite 作为持久化容器，启用 WAL，把文档主体作为 JSON snapshot 存在表里，同时抽出少量高频索引字段。MyOpenPanels 这次升级沿用这个方向，但要保留当前 panel/runtime 抽象。

## 目标

1. 使用 SQLite 管理 MyOpenPanels 项目元数据、panel 状态、artifact、selection、wiki task 等结构化数据。
2. 用 migration 管理数据库 schema 版本，所有结构升级都通过迁移脚本完成。
3. 保持现有 `MyOpenPanelsStorage` / `MyOpenPanelsRuntime` 对外接口基本不变，优先替换 local storage 实现。
...
本地存储层从 `LocalMyOpenPanelsStorage` 升级为 SQLite-backed 实现。外部调用仍然使用：
...
- `crates/myopenpanels/src/storage.rs` 封装 SQLite connection、PRAGMA、migration、typed queries。
...
MyOpenPanels 的本地 CLI/server 已迁移到 Rust。当前 storage 实现使用 Rust `rusqlite`：
...
```ts
export class LocalMyOpenPanelsStorage implements MyOpenPanelsStorage {
  constructor(options) {
...
当前 Rust 实现在 `crates/myopenpanels/src/storage.rs` 内维护 migration registry。
每条 migration 包含：
...
- `panel_ids_json` 第一阶段保留，方便维持当前 `MyOpenPanelsSession.panelIds` 数据形态；后续可完全由 `panels` 表派生。
...
`crates/myopenpanels/src/control.rs` 和 `crates/myopenpanels/src/wiki.rs`
...
- `myopenpanels panel list --format json`
- `myopenpanels panel current --format json`
- `myopenpanels panel switch --kind <kind> --format json`
- `myopenpanels wiki context --format json`
- `myopenpanels canvas state --format json`
...
- 建立初始 schema。
- `LocalMyOpenPanelsStorage` 改为 DB-backed。
- 保持 assets 文件逻辑不变。
