...
- 不重写 runtime、local-control、studio 的业务模型。
...
```text
.myopenpanels/
  myopenpanels.sqlite3
  myopenpanels.sqlite3-wal
  myopenpanels.sqlite3-shm
...
- 新增 `packages/local-storage/src/sqlite.ts`，封装 SQLite connection、PRAGMA、migration、typed queries。
- `LocalOpenPanelsStorage` 默认使用 SQLite。
- 不保留 JSON file storage 作为运行时兼容层。
- 如果 `.myopenpanels/myopenpanels.sqlite3` 不存在，直接创建全新数据库并执行 migrations。

## SQLite Driver

OpenPanels 是 TypeScript/Node 项目，不能直接复用 CreartCLI 的 Rust `sqlx`。当前实现使用 Node 提供的 `node:sqlite` / `DatabaseSync`：

- 不引入额外 native npm dependency。
...
- 当前 storage interface 已经是 async，内部使用同步 driver 不影响外部接口；后续也可以替换 driver。

注意：`node:sqlite` 在当前 Node 版本中仍会打印 ExperimentalWarning。后续如果发布目标需要更低 Node 版本或更稳定的 sqlite 绑定，可以在 storage 层内部替换为 `better-sqlite3`，不改变 runtime/storage 对外接口。
...
export class LocalOpenPanelsStorage implements OpenPanelsStorage {
  constructor(options) {
    ...
    this.dbPath = join(this.rootDir, "myopenpanels.sqlite3")
...
```text
packages/local-storage/src/migrations/
  0001_initial.ts
...
1. 首次打开 storage 时创建 `.myopenpanels/myopenpanels.sqlite3`。
...
`packages/local-control` 里不少功能现在直接依赖文件路径。升级策略是“状态进 SQLite，大文件路径保留”：

- `getProjectBootstrap()` / `getWikiBootstrap()` 继续通过 runtime/storage 读取 session、panel、state。
...
这些路径不再作为项目状态的数据源。新代码只能通过 storage/runtime API 读写结构化状态。
...
```json
{
  ...
  "databasePath": ".../.myopenpanels/myopenpanels.sqlite3",
  "databaseExists": true,
...
当前实现依赖 Node 的内置 `node:sqlite`。它避免了额外 native dependency，但运行时需要支持 `node:sqlite` 的 Node 版本，并且测试/运行时会出现 ExperimentalWarning。storage 层已经把 driver 隐藏在 `LocalOpenPanelsStorage` 内部，后续可以替换 driver。

## 已确认决策

1. SQLite DB 文件名使用 `myopenpanels.sqlite3`。
