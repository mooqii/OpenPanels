...
> superseded by Protocol v2 `agent bootstrap`.

## 背景

MyOpenPanels 已经具备 Project 内多 panel 的基础形态，`wiki` panel 目前是占位 UI，`canvas` panel 已经可独立持久化和通过 CLI 被 agent 操作。本 spec 描述下一阶段的 wiki 面板：把用户或 agent 添加的原始文档整理为可编辑 Markdown，再持续沉淀为 agent 友好的结构化 wiki。
...
“立即主动 push 给 agent”是 v1 的必需能力：用户或 agent 添加新文档后，MyOpenPanels 必须唤醒当前可用的 agent thread，让 agent 立即开始处理。持久任务队列仍然是可靠性基础，但不能替代真实唤醒。
...
myopenpanels wiki agent-target register --host <host> --thread-id <id>
myopenpanels wiki raw add --file <path> [--wiki-space-id <id>]
myopenpanels wiki raw new-markdown --title <title> --file-name <name> [--wiki-space-id <id>]
myopenpanels wiki raw add-text --title <title> --file-name <name> [--wiki-space-id <id>]
myopenpanels wiki raw list --format json
myopenpanels wiki raw read --document-id <id> --output <path>
myopenpanels wiki markdown read --document-id <id>
myopenpanels wiki markdown write --document-id <id> --file <path> --task-id <id>
myopenpanels wiki tasks list --status queued
myopenpanels wiki tasks next
myopenpanels wiki tasks claim --task-id <id>
myopenpanels wiki tasks complete --task-id <id>
myopenpanels wiki tasks fail --task-id <id> --message <message>
myopenpanels wiki processes list
myopenpanels wiki processes read --process-id <id>
myopenpanels wiki rules list
myopenpanels wiki rules read --rule-set-id <id>
myopenpanels wiki spaces list
myopenpanels wiki spaces active --wiki-space-id <id>
myopenpanels wiki pages list --wiki-space-id <id>
myopenpanels wiki pages create --wiki-space-id <id> --path <path> --file <path>
myopenpanels wiki pages read --wiki-space-id <id> --path index.md
myopenpanels wiki pages write --wiki-space-id <id> --path index.md --file <path> --task-id <id>
```

`agent context` 需要在 active panel 是 wiki 或存在 pending wiki tasks 时加入：
