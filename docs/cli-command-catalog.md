...
Target modes distinguish `panel-kind`, `active-selection`, `task-bound`, and
`operation-bound`. Panel reads and writes target their resource without changing
the user's focus. Only commands that inspect or export the current selection use
the active panel.
