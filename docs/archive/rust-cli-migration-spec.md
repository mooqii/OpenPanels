# MyOpenPanels Rust CLI Migration Spec
...
MyOpenPanels 迁移前的 `myopenpanels` 由 TypeScript/Node.js 实现，运行时依赖
用户机器上的 Node.js。
...
- 内部本地 HTTP server：`__serve-studio` 启动 API 服务并托管
  `apps/studio/dist` 静态资源。

参考 `CreartCLI` 的方向，MyOpenPanels 下一阶段要把本地 CLI 和本地服务迁移为
Rust 原生二进制。最终用户安装后不再需要 Node.js runtime 即可运行
`myopenpanels`。React/Vite studio 前端继续保留 TypeScript 实现，只在
release 构建时产出静态资源并嵌入 Rust binary。

## 目标

1. 交付一个 Rust 原生 `myopenpanels` binary，覆盖旧 CLI 的用户可见命令。
...
6. 保留 `apps/studio` 前端；canvas 作为 studio 内部模块维护。
7. 使用 GitHub Releases 发布多平台二进制。
8. 安装以 GitHub Releases binary 和安装脚本为准。

## 非目标

- 不把 studio UI、canvas editor、React 组件迁移到 Rust。
- 不在本阶段改变 MyOpenPanels 产品模型、panel schema 或 wiki 业务逻辑。
...
- CLI 命令协议以迁移前的 TypeScript CLI 为迁移基准，不借迁移扩大命令集；当前
  实现权威已经转到 `crates/myopenpanels/src/cli.rs`。
- 前端数据类型保留在 `apps/studio/src/protocol.ts`。
- SQLite schema 以 `crates/myopenpanels/src/storage.rs` 为权威实现。
- `agent-capabilities` 应迁移为语言无关 manifest，避免 Rust 和 TypeScript 重复维护。
- Release binary 内嵌 `apps/studio/dist`，同时保留
  `MYOPENPANELS_STUDIO_STATIC_DIR` 作为开发和调试覆盖入口。
...
当前不再保留 Node CLI；发布入口是 GitHub Releases 中的原生二进制。
...
myopenpanels agent targets list|register|heartbeat|remove
myopenpanels agent guides
myopenpanels agent guide <id>
myopenpanels agent skills
myopenpanels agent skill <id>

myopenpanels project current|list|create
myopenpanels panel list|current|switch
myopenpanels canvas state
myopenpanels canvas selection read|export
myopenpanels canvas placeholder create
myopenpanels canvas image insert
myopenpanels tasks list|next|inspect|claim-next|claim|heartbeat|complete|fail|release|retry|cancel|deliveries

myopenpanels wiki context
myopenpanels wiki selection read
myopenpanels wiki documents list|add|create-markdown
myopenpanels wiki markdown read
myopenpanels wiki markdown write
myopenpanels wiki tasks list|next|claim|complete|fail
myopenpanels wiki spaces list|switch
myopenpanels wiki pages list|search|read|write

myopenpanels update check|download|install
...
在 `crates/myopenpanels/` 引入 Rust crate，保持仓库根目录继续作为 pnpm
monorepo 和产品文档入口：
...
binary 名称必须是 `myopenpanels`。根目录 `Cargo.toml` 只作为 Rust workspace
入口，避免把 CLI crate、pnpm workspace、前端包的职责混在同一层。

### Rust 模块职责

- `main.rs`：收集 argv，调用 `myopenpanels::run_cli`，按返回码退出。
- `cli.rs`：命令解析、格式化输出、exit code、alias、参数校验。
- `types.rs`：MyOpenPanels session、panel、artifact、wiki、selection、CLI JSON 输出类型。
- `paths.rs`：解析 project/storage/context 路径和环境变量。
- `storage.rs`：SQLite-backed storage，实现当前 `LocalMyOpenPanelsStorage` 行为。
- `migrations.rs`：Rust 版本 SQLite migrations 和 checksum/版本记录。
- `control.rs`：迁移当前 `@myopenpanelsRust control/wiki modules` 的 project/panel/bootstrap 业务逻辑。
...
- `rusqlite` 更适合当前同步 SQLite 使用方式；如果未来需要 async pool，可再切换
  `sqlx`。
- `include_dir` 用于嵌入 `apps/studio/dist`、`agent-resources` 和 capabilities
  manifest。
...
其中 canvas 已内联到 `apps/studio/src/canvas`，前端协议类型位于
`apps/studio/src/protocol.ts`。Rust 侧维护等价 `serde` 类型。
...
当前第一版直接由 `crates/myopenpanels/src/agent.rs` 维护 capability 和
...
GitHub Releases，不依赖 MyOpenPanels 云服务。
...
```text
scripts/install-myopenpanels.sh
scripts/install-myopenpanels.ps1
```
...
3. 校验 checksum。
4. 解压为 `myopenpanels` 或 `myopenpanels.exe`。
5. 安装到用户 PATH 中的目录，例如 `~/.local/bin`。
6. 输出 `myopenpanels --version` 验证结果。
...
- 明确 `resolveMyOpenPanelsPaths`、context ID、session JSON 的稳定规则。
- 为 SQLite schema 加 Rust 可复用 schema 文档。

### Phase 1: Rust Skeleton

- 新增根目录 Rust workspace 和 `crates/myopenpanels/Cargo.toml`。
- 新增 `crates/myopenpanels/src/main.rs`、`src/lib.rs`。
...
- 增加 `scripts/install-myopenpanels.sh` 和
  `scripts/install-myopenpanels.ps1`。
- 更新 README、docs、skill。
- 安装脚本直接下载 GitHub Releases native binary。
