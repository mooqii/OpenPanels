...
- runtime 已经有 `PanelRegistry`，可以按 `kind` 注册不同 panel。

当前限制主要在产品入口和命名上：本地 studio、CLI、local-control helper、agent skill 都仍然以 canvas 为中心。增加 wiki 面板时，应该顺手把产品模型升级为：
...
- 如需要，可导出 `wikiStateSchema`，但初期可以先只在 local-control/core 里约定状态。
...
文件：`packages/local-control/src/index.ts`
...
文件：`packages/local-cli/src/index.ts`
...
```bash
npx -y @openpanels/local-cli@latest agent-context --project "$PWD" --format markdown
```
...
```json
{
  ...
  "commands": {
    "studioStart": "npx -y @openpanels/local-cli@latest studio start --project \"$PWD\" --format json",
    "agentContext": "npx -y @openpanels/local-cli@latest agent-context --project \"$PWD\" --format markdown"
  },
  "panelInstructions": {
    "wiki": "...",
    "canvas": "..."
...
1. 使用 `npx -y @openpanels/local-cli@latest` 或确保全局 `openpanels-local` 最新。
2. 启动 studio。
3. 调用 `agent-context`。
...
- `local-control`: `ensureProjectBootstrap` creates/loads both wiki and canvas.
- `local-control`: old canvas bootstrap wrapper still returns canvas panel.
- `local-cli`: new commands return stable JSON.
...
- CLI source of truth: `agent-context` generated by latest npm CLI.
