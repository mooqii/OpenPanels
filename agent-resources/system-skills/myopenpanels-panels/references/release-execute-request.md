...
3. Use only the permitted browser destination or exact fenced official API
   command declared by the Runtime Contract, and preserve the captured content
   verbatim. Stop with `needs_user_action` when required account configuration,
   authentication, verification, or platform permission blocks progress.
